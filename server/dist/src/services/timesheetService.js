...
export async function updateTimesheetService({ id, editorId, changes, changeReason, numberOfChanges, startTime, endTime, Jobsite, CostCode, comment, }) {
    try {
        const parsedChanges = changes && typeof changes === "string"
            ? JSON.parse(changes)
            : changes || {};
...
export async function getTimesheetDetailsManager({ timesheetId, }) {
...
export async function getManagerCrewTimesheets({ managerId, }) {
...
export async function approveTimesheetsBatchService({ userId, timesheetIds, statusComment, editorId, }) {
    try {
...
export async function createTascoTimesheetService({ data, type, }) {
...
export async function getRecentTimeSheetForUser(userId) {
...
export async function getTimesheetActiveStatus({ userId }) {
...
export async function getBannerDataForTimesheet(timesheetId, userId) {
...
export async function getLogsForDashboard(userId) {
    ...
    const validateField = {
        ...
        mileage: (value, startingMileage) => {
            if (!validateField.required(value))
                return false;
            if (startingMileage === null)
                return true; // Can't validate without starting mileage
    ...
    const validateTruckingLog = (log) => {
        ...
        return {
            ...
            stateMileage: log.StateMileages.some((item) => !validateField.required(item.state) ||
                !validateField.mileage(item.stateLineMileage, startingMileage)),
            // Refuel logs validation
            refueled: log.RefuelLogs.some((item) => !validateField.positiveNumber(item.gallonsRefueled) ||
                !validateField.mileage(item.milesAtFueling, startingMileage)),
            // Material validation
            material: log.Materials.some((item) => !validateField.required(item.LocationOfMaterial) ||
            ...
            equipmentHauled: log.EquipmentHauled.some((item) => !validateField.required(item.equipmentId)),
    ...
    const mappedEmployeeLogs = employeeLogs.map((log) => ({
        id: log.id.toString(),
        type: "equipment",
        userId: userId,
    ...
    const mappedTruckingLogs = truckingLogs
        ...
        return (log.incomplete ||
            log.stateMileage ||
            log.refueled ||
            log.material ||
            log.equipmentHauled);
    });
    const mappedTascoLog = tascoLogs
        .map((log) => {
        return {
            id: log.id,
            type: "tasco",
            ...
            refueled: log.RefuelLogs.some((item) => !validateField.positiveNumber(item.gallonsRefueled)),
            fLoads: log.TascoFLoads.some((item) => !validateField.positiveNumber(item.weight) ||
...
export async function getClockOutComment(userId) {
    const timesheet = await prisma.timeSheet.findFirst({
        where: {
            userId,
            endTime: null, // Ensure timesheet is still active
...
export async function getRecentJobDetails(userId) {
    const timesheet = await prisma.timeSheet.findFirst({
        where: {
            userId,
            endTime: null, // Ensure timesheet is still active
    ...
    try {
        ...
        if (!equipmentExists) {
            ...
            if (equipmentAnyStatus) {
                throw new Error(`Equipment with ID ${equipmentId} is ${equipmentAnyStatus.status.toLowerCase()}. Please scan an active equipment QR code.`);
            }
            else {
                throw new Error(`Equipment with ID ${equipmentId} not found. Please scan a valid equipment QR code.`);
            }
        }
        ...
        if (timeSheet.endTime) {
            throw new Error("This timesheet has been closed. Please clock in again before logging equipment.");
        }
...
export async function getEmployeeEquipmentLogDetails(logId) {
...
export async function deleteEmployeeEquipmentLog(logId) {
    try {
...
export async function updateEmployeeEquipmentLogService({ id, equipmentId, startTime, endTime, comment, status, disconnectRefuelLog, refuelLogId, gallonsRefueled, }) {
    try {
        const result = await prisma.$transaction(async (prisma) => {
            const actualRefuelLogId = refuelLogId === "__NULL__" ? null : refuelLogId;
            const actualGallonsRefueled = gallonsRefueled === null ? null : gallonsRefueled;
            let refuelLogUpdate = {};
            ...
            return {
                ...
                createdBy: log.TimeSheet && log.TimeSheet.User
...
export async function getClockOutDetailsService(userId) {
...
export async function updateClockOutService(timeSheetId, userId, endTime, timeSheetComments, wasInjured, clockOutLat, clockOutLong) {
    try {
...
export async function getPreviousTimesheet(userId) {
    const timesheet = await prisma.timeSheet.findFirst({
        where: {
            userId,
        },
...
export async function getContinueTimesheetService(id, userId) {
...
export async function returnToPreviousTimesheetService(id) {
 ...
 */
export async function deleteRefuelLogService(refuelLogId) {
    try {
